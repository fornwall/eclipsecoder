...
public class TopCoderAppletLauncher {
					...
					new AWTEventListener() {
						public void eventDispatched(AWTEvent event) {
							if (!(event.getSource().getClass().getName()
									.equals("com.topcoder.client.contestApplet.frames.MainFrame")))
								return;
	...
	static void fixVerify(Object mainFrame) {
		Object contestApplet = ReflectUtil.invokeInstanceMethod(mainFrame,
				"getContestApplet");
		Object contestant = ReflectUtil.invokeInstanceMethod(contestApplet,
				"getModel");
	...
	static void logon(Object mainFrame, String userName, String userPassword) {
		Object contestApplet = ReflectUtil.invokeInstanceMethod(mainFrame,
				"getContestApplet");
		Object roomManager = ReflectUtil.invokeInstanceMethod(contestApplet,
				"getRoomManager");
		Object currentRoom = ReflectUtil.invokeInstanceMethod(roomManager,
				"getCurrentRoom");

		// work around TopCoder check for classes pressing the login button
		Object contestant = ReflectUtil.invokeInstanceMethod(contestApplet,
				"getModel");
		String[] validIds = (String[]) ReflectUtil
				.getField(contestant, "valid");
		...
		JTextField nameInput = (JTextField) ReflectUtil.getInstanceField(
				ReflectUtil.getInstanceField(currentRoom, "userName", true),
				"component", false);
		JPasswordField passwordInput = (JPasswordField) ReflectUtil
				.getInstanceField(ReflectUtil.getInstanceField(currentRoom,
						"passWord", true), "component", false);
		final JButton loginButton = (JButton) ReflectUtil.getInstanceField(
				ReflectUtil.getInstanceField(currentRoom, "loginButton", true),
				"component", false);
	...
	private static void startContestAppletFirstTime(final ClassLoader loader) {
		...
		System
				.setProperty(
						"com.topcoder.client.contestApplet.common.LocalPreferences.filelocation",
					...
					new AWTEventListener() {
						public void eventDispatched(AWTEvent event) {
							try {
								...
								final JFrame mainFrame = (JFrame) event
										.getSource();
								...
								mainFrame
										.addWindowListener(new WindowAdapter() {
											...
											public void windowClosed(
												...
												StartTopCoderAppletAction.getAction()
														.setEnabled(true);
											}
										});
								...
								Object pluginManager = getInstanceMethod
										.invoke(null);
								...
								Constructor<Object> constructorMethod = editorPluginClass
										.getConstructor(String.class,
												...
												boolean.class);
								Object editorPlugin = constructorMethod
										.newInstance("EclipseCoder",
												eclipseCoderEntryPointClass
														.getName(), "", false);
								...
								Method setEagerMethod = editorPlugin.getClass()
										.getMethod("setEager", boolean.class);
								setEagerMethod.invoke(editorPlugin, true);
								// editorPlugin.setEager(true);

								String classPath = StartTopCoderAppletAction.class
										.getProtectionDomain().getCodeSource()
										.getLocation().getPath();

								Method setClassPathMethod = editorPlugin
										.getClass().getMethod("setClassPath",
												String.class);
								setClassPathMethod.invoke(editorPlugin,
										classPath);
								// editorPlugin.setClassPath(classPath);

								Object editorPluginsArray = Array.newInstance(
										editorPluginClass, 1);
								Array.set(editorPluginsArray, 0, editorPlugin);
								Method setEditorPluginsMethod = pluginManager
										.getClass().getMethod(
												"setEditorPlugins",
												editorPluginsArray.getClass());
								setEditorPluginsMethod.invoke(pluginManager,
										editorPluginsArray);
								...
								getInstanceMethod = localPreferencesClass
										.getMethod("getInstance");
								Object localPreferencesInstance = getInstanceMethod
										.invoke(null);
								Method setDefaultEditorNameMethod = localPreferencesInstance
										.getClass().getMethod(
												"setDefaultEditorName",
												String.class);
								setDefaultEditorNameMethod.invoke(
										localPreferencesInstance,
										"EclipseCoder");
								// LocalPreferences.getInstance().setDefaultEditorName("EclipseCoder");

								Field cacheField = pluginManagerClass
								...
								Field editorField = dynamicEditorClass
										.getDeclaredField("editor");
								editorField.setAccessible(true);

								Object eclipseCoderEntryPoint = eclipseCoderEntryPointClass
										.newInstance();
								editorField.set(ownEditor,
										eclipseCoderEntryPoint);
								...
								// replaced
								Field methodCacheField = dynamicEditorClass
								...
								Map<String, Method> methodCache = (Map<String, Method>) methodCacheField
										.get(ownEditor);
								for (Method m : eclipseCoderEntryPointClass
										.getMethods()) {
			...
			mainMethod
					.invoke(
							...
							new Object[] { new String[] {
									"www.topcoder.com",
									"5001",
									"http://tunnel1.topcoder.com/dummy?tunnel",
									"TopCoder" } });
		} catch (Exception e) {
			Utilities.showException(e);
